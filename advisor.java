...
// Client class to initialize the GUI
	...
	BufferedReader in;
	PrintWriter out;
	JFrame frame = new JFrame("advisor");
	JTextArea messageArea = new JTextArea(15, 50);

	Map<String, String> advisor = new HashMap<String, String>(); // HashMap to store the Clients registered with the Server

	public advisor() { // Constructor to initialize the Client
...
//	}
	...
	class SayHello extends TimerTask {
		    public void run() {
				...
				out.println("POLL,POST /localhost/server http/1.1,Host:" + serverAddress 
...
//					out.println(getName()); // Send the desired screen name to the server for acceptance
				...
				// block of code is executed if message is received from another client
    				...
    				if(Math.abs(randomValue) == 1)
    					acc = "Approved";
    				else
    					acc = "Disapproved";
    				String data = "MESSAGE, Student Name: "+line.split(",")[1]+", Subject Name: "+line.split(",")[2]+", Advisor Name: "+line.split(",")[3]+", ACCEPTENCE : "+acc+"\n";
    				messageArea.append(data.split(",")[1]+","+data.split(",")[2]+","+data.split(",")[3]+","+data.split(",")[4]+"\n");
    				out.println(data);
				}
	...
	public static void main(String[] args) throws Exception {
		...
		advisor.frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		advisor.frame.setVisible(true);
		advisor.run();
	}
}